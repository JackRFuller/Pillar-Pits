...
public class UnityDataConnector : MonoBehaviour
{
    ...
    public static dataSheet currentDataSheet;
    public dataSheet userDataSheet;

	[HideInInspector] public string webServiceUrl = "";
	[HideInInspector] public string spreadsheetId = "";
	[HideInInspector] public string worksheetName = "";
	[HideInInspector] public string password = "";
	[HideInInspector] public float maxWaitTime = 10f;
	[HideInInspector] public GameObject dataDestinationObject;
	[HideInInspector] public string statisticsWorksheetName = "Statistics";
	...
	void Start ()
	{
        currentDataSheet = userDataSheet;

        if(userDataSheet == dataSheet.Jack)
        {
            spreadsheetId = "1WY_B7F1ff5kkT__W3CpZcphPbdixRM_DwiARIqaaKIo";            
        }

        if (userDataSheet == dataSheet.Chris)
        {
            spreadsheetId = "1TqA_1Pva5m4MdGRLxyYV9mtvbtAyU4xddYi9m_C45_Y";          
        }
	...
	IEnumerator GetData()
	{
		...
		dataDestinationObject.SendMessage("AssignValues", ssObjects);
	}
